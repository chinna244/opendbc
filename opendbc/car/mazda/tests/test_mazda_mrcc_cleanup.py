#!/usr/bin/env python3
"""Proven TJA-caused MRCC cleanup, adapted to the upstream TJA_BUTTON gate."""
...
class TestTjaIcbmSuppressScoping:
  """ICBM must ignore the physical TJA bit unless TJA_BUTTON is set."""
  ...
  def _cc(candidate, *, car_fw=None):
    ...
    return CarController({Bus.pt: "mazda_2017"}, CP, CP_SP), CP_SP
  ...
  def test_tja_button_suppresses_icbm_while_tja_held(self):
    cc, CP_SP = self._cc(CAR.MAZDA_CX5_2022)
    assert CP_SP.flags & MazdaFlagsSP.TJA_BUTTON
  ...
  def test_non_tja_does_not_suppress_icbm_for_tja_bit(self):
    ...
    for candidate, car_fw in (
      ...
      cc, CP_SP = self._cc(candidate, car_fw=car_fw)
      assert not (CP_SP.flags & MazdaFlagsSP.TJA_BUTTON)
      CC = structs.CarControl()
      CC.latActive = False
      CC = CC.as_reader()
      CC_SP = structs.CarControlSP()
...
class TestTjaMrccSideEffect:
  ...
  def test_exact_cleanup_wire_bytes(self):
    cc = self._cc()
    CC, CC_SP = self._controls()
    ...
    payloads = self._mrcc_off_payloads(
      self._step_to_first_tx_deadline(cc, CC, CC_SP, tja=0, armed=True, raw_armed=True, counter=3))
    for counter in (4, 5):
      payloads.extend(self._mrcc_off_payloads(
        self._step(cc, CC, CC_SP, tja=0, armed=True, raw_armed=True, counter=counter)))
    assert [dat.hex() for dat in payloads] == [
      "0081fed000000000",
      "0081fed400000000",
      "0081fed800000000",
    ]

  def test_cleanup_helper_is_mrcc_master_only(self):
    _, dat, _ = mazdacan.create_mrcc_off_cmd(self._cc().packer, 3)
    assert dat.hex() == "0081fed000000000"

  @pytest.mark.parametrize("mads_active", [False, True])
  def test_pre_off_cleanup_is_independent_of_mads_direction(self, mads_active):
    cc = self._cc()
    CC, CC_SP = self._controls()
    ...
    assert len(self._mrcc_off_payloads(
      self._step_to_first_tx_deadline(cc, CC, CC_SP, tja=0, armed=True, raw_armed=True, counter=3))) == 1

  @pytest.mark.parametrize("mads_active", [False, True])
  @pytest.mark.parametrize("cruise_enabled", [False, True], ids=["armed", "active"])
  def test_preexisting_mrcc_is_preserved_in_both_mads_states(self, mads_active, cruise_enabled):
    cc = self._cc()
    CC, CC_SP = self._controls()
    CC_SP.mads.active = mads_active
    self._step(cc, CC, CC_SP, tja=0, armed=True, raw_armed=True, counter=3,
