...
class CarController(CarControllerBase, IntelligentCruiseButtonManagementInterface):
  def __init__(self, dbc_names, CP, CP_SP):
    ...
    self.tja_mrcc_retry_active = False
    self.tja_mrcc_unarm_frames = 0
    self.tja_mrcc_raw_off_frames = 0
    self.tja_mrcc_last_counter: int | None = None
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    # Synchronize each attempt to a newly received wheel-button counter: sending on a
    # fixed 10 Hz phase can race the OEM frame with the same counter, making Mazda reject
    # our later duplicate. Preserve MRCC only when it was genuinely armed before TJA.
    if has_tja_mads(self.CP):
      tja_button = bool(getattr(CS, "tja_button", 0))
      crz_btns_counter = int(CS.crz_btns_counter)
      crz_btns_counter_changed = crz_btns_counter != self.tja_mrcc_last_counter
      self.tja_mrcc_last_counter = crz_btns_counter
      ...
      if tja_pressed:
        # A failed cleanup remains our responsibility across later TJA presses. Do not
        # reinterpret the MRCC state caused by the earlier press as driver-prearmed.
      ...
      if self.tja_mrcc_unarm_pending:
        ...
        if self.tja_mrcc_saw_armed and raw_off_confirmed:
          ...
          # A long hold must not consume the retry window. Start or restart it only on
          # release, including when a previous attempt timed out but still owns the arm.
          if tja_released:
            self.tja_mrcc_retry_active = True
            self.tja_mrcc_unarm_frames = 0

          retry_unblocked = not CC.cruiseControl.cancel and not CC.cruiseControl.resume
          if self.tja_mrcc_retry_active and not tja_button and retry_unblocked:
            self.tja_mrcc_unarm_frames += 1
            if self.tja_mrcc_unarm_frames > TJA_MRCC_UNARM_TIMEOUT_FRAMES:
              self.tja_mrcc_retry_active = False
              # If TJA never armed MRCC there is no side effect to own. Otherwise retain
              # the debt so a later TJA release retries instead of preserving it as OEM.
              if not self.tja_mrcc_saw_armed:
                self.tja_mrcc_unarm_pending = False
            elif self.tja_mrcc_saw_armed and crz_btns_counter_changed:
    ...
    # TJA_MADS only: also suppress while physical TJA is held or an MRCC cleanup retry is
    # active, so ICBM cannot fabricate a TJA edge or race the counter-synchronized tap.
    # A dormant cleanup debt must not suppress ICBM forever after its bounded retry window.
    icbm_suppress = (
      CC.cruiseControl.cancel or CC.cruiseControl.resume or CS.cancel_button == 1 or
      (has_tja_mads(self.CP) and (getattr(CS, "tja_button", 0) == 1 or self.tja_mrcc_retry_active))
    )
