...
class TestMazdaMrccCleanupSafety(unittest.TestCase):
  ...
  def test_raw_regression_family(self):
    self._rx_mrcc(True)
    for counter in (4, 5, 6):
      msg = self._button(CTR=counter)
      self.assertEqual(bytes(msg.data)[:8].hex(), f"0081fe{counter << 2 | 0xc0:02x}00000000")
  ...
  def test_cleanup_composites_are_rejected(self):
    self._rx_mrcc(True)
    composites = (
      {"SET_P": 1, "SET_P_INV": 0}, {"SET_M": 1, "SET_M_INV": 0},
      {"RES": 1, "RES_INV": 0}, {"CAN_OFF": 1, "CAN_OFF_INV": 0},
      {"TJA_BUTTON": 1}, {"MODE_X": 1, "MODE_X_INV": 0},
      {"MODE_Y": 1, "MODE_Y_INV": 0}, {"DISTANCE_LESS": 1, "DISTANCE_LESS_INV": 0},
